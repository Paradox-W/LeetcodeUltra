...
export class GenericVisualizationBackend extends VisualizationBackendBase {
	...
	> {
		...
		try {
			...
			if (reply.variablesReference) {
				const resultFromVariableChildren =
					await this.tryParseVisualizationDataFromVariableChildren(
						reply.variablesReference
					);
				if (resultFromVariableChildren) {
					return resultFromVariableChildren;
				}
		...
		private async tryParseVisualizationDataFromVariableChildren(
	...
	> {
		...
		const candidates = variables
			...
			.sort(
				(a, b) =>
					this.getStringDataCandidatePriority(a.name) -
					this.getStringDataCandidatePriority(b.name)
			);
	...
	private getStringDataCandidatePriority(name: string): number {
		if (/^(data|__data_|_data)$/i.test(name)) {
			return 0;
		}
		if (/data/i.test(name)) {
			return 1;
